...
<body>
	...
	<div class="container mt-5 mb-5">
		<div class="d-flex justify-content-center row">
			<div class="col-md-8">
				...
				<div
					...
					<div class="mr-1">
						<img
							src="./GetPictureServlet?Product_ID=<%=p.getProdotto().getProduct_ID()%>"
							onerror="this.src='./imgs/nophoto.png'" style="width: 70px">
					</div>
					<div class="d-flex flex-column align-items-center product-details">
						<span class="font-weight-bold"><%=p.getProdotto().getProduct_Name()%></span>
						<div class="d-flex flex-row product-desc">
							<div class="size mr-1">
								<span class="text-grey">Model:</span><span
									class="font-weight-bold">&nbsp;<%=p.getProdotto().getModel()%></span>
							</div>
							<div class="color">
								<span class="text-grey">Brand:</span><span
									class="font-weight-bold">&nbsp;<%=p.getProdotto().getBrand()%></span>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center qty">
						...
						<h5 class="text-grey mt-1 mr-1 ml-1">
							<input type="number" min=1 max=<%=p.getProdotto().getQuantity()%>
								value=1 class="quantity"
								data-price="<%=p.getProdotto().getPrice()%>">
						</h5>
					...
					<div>
						<form
							action="./RemoveFromCart?Product_ID=<%=p.getProdotto().getProduct_ID()%>"
							method="post">
							<h5 class="text-grey">
		...
		<div
			...
			<input type="text" class="form-control border-0 gift-card"
				placeholder="discount code/gift card"
				value="Il totale da pagare per il tuo ordine è di <%=total%> euro"
				style="text-align: center; font-weight: 900;" disabled>
		</div>
		<div
			class="d-flex flex-row align-items-center mt-3 p-2 bg-white rounded">
